...
namespace std {
    ...
    public:
        ...
        Type(bool isRoot, const vector<vector<vector<double>>> & initialVals, const pair<string,vector<pair<int,int>>> & theType) {
            ...
            this->right = nullptr;
            this->counter = 0;
            this->name = theType.left;
            this->indices = theType.right;
        ...
        vector<vector<branchInfo>> initVals(const vector<vector<vector<double>>> & initialVals) {
            if(this->isRoot) {
        ...
        pair<bool,branchInfo> getNextVal(int whichBranch, bool random) {
            ...
            if(this->counter == this->vals[whichBranch].size()) {
                ...
                branchInfo ans = this->vals[whichBranch][this->counter];
            }
